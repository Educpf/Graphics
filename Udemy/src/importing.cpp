...
int main() {
    ...
    formula = Model();
    formula.LoadModel("Models/formula.obj");
    ...
    spotLightCount++;
    spotLights[1] = SpotLight(0.4f, 1.0f, 0.4f,   // color
                                0.0f, 2.0f,        // ambient and diffuse intensity
                                -2.3f, 4.10f, -12.37f, // position
                                0.83f, -0.256f, 0.49f, // direction
                                1.0f, 0.0f, 0.0f,  // distance attenuation
                                30.0f);             // edge
    CreateObject();
    CreateShaders();
    ...
    while (!mainWindow.getShouldClose()) {
        ...
        std::cout << camera.getCameraDirection().x << "- " << camera.getCameraDirection().y << "- " << camera.getCameraDirection().z << "\n";
    }
    return 0;
}
